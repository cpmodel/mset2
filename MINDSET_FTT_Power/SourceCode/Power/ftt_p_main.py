...
from MINDSET_FTT_Power.SourceCode.ftt_core.ftt_sales_or_investments import get_sales, get_sales_yearly
from MINDSET_FTT_Power.SourceCode.Power.ftt_p_rldc import rldc
from MINDSET_FTT_Power.SourceCode.Power.ftt_p_dspch import dspch
from MINDSET_FTT_Power.SourceCode.Power.ftt_p_lcoe import get_lcoe, set_carbon_tax


from MINDSET_FTT_Power.SourceCode.Power.ftt_p_surv import survival_function
...
# ----------------------------- Main ------------------------------------------
# -----------------------------------------------------------------------------
def solve(data, time_lag, iter_lag, titles, conv, histend, year, domain):
    """
    Main solution function for the module.
    ...
    tech_to_resource = [titles['ERTI'].index(t) for t in conv['T2TI_ERTI'].loc[list(titles['T2TI']), 'ERTI']]
    # tech_to_resource = [0, 1, 2, 2, 2, 2, 3, 3, 4, 4, 4, 4, 5, 6, 7, 8, 9, 10, 11, 11, 12, 13, 3, 3]
    # MEWD electricity index
    ...
    data['FPIX'] = time_lag['FPIX'] * (1 + data['FPI'])
    ...
    if year == _EX_BASE_YEAR:
        ...
        bcet, bcsc, mewl, mepd, merc, rery, mred, mres = cost_curves(
                ...
                titles['RTI'], titles['T2TI'], titles['ERTI'], year, 1.0, tech_to_resource
                )
        ...
        tot_elec_dem = data['MEWG'][:, :, 0].sum(axis = 1) # data['MEWDX'][:, elec_idx,0] * 1000/3.6
    ...
    # Up to the last year of historical market share data
        ...
        data['IMP'][:, 0, 0] = data['MEWG'][:, :, 0].sum(axis = 1) / (data['MEWDX'][:, elec_idx, 0] * 1000/3.6)
        data['MEWD'][:, elec_idx, 0] *= data['IMP'][:, 0, 0]
        data['MEWDX'][:, elec_idx, 0] *= data['IMP'][:, 0, 0]
...
#        data['MEWL'][:, :, 0] = np.copy(loadfac)
        ...
        if year >= _RLDC_START_YEAR:
            ...
            # Total electricity demand
            tot_elec_dem = data['MEWDX'][:, elec_idx, 0] * 1000/3.6 # data['MEWG'][:, :, 0].sum(axis = 1)
            ...
            for r in range(len(titles['RTI'])):
                ...
                gross_curt_e= data['MCRT'][r,0,0] * np.sum(Svar[r, :] * data['MEWG'][r,:,0])
                data['MCGA'][r, 0, 0] = np.maximum(gross_curt_e, 0.000001)
...
#        data['MWIA'][:, 0, 0][data['MEWKA'][:, 0, 0] < 0.0] = 0.0
        ...
        # Copy over import share from previous period
        data['IMP'][:, 0, 0] = time_lag['IMP'][:, 0, 0]
        data['MEWD'][:, elec_idx, 0] *= data['IMP'][:, 0, 0]
        data['MEWDX'][:, elec_idx, 0] *= data['IMP'][:, 0, 0]
...
#                data['MEWK'][r, :, 0] = divide(data['MEWG'][r, :, 0], data['MEWL'][r, :, 0])/8766
            ...
            data['MCGA'][:,0,0][data['MCGA'][:,0,0] == 0] = 0.000001
            ...
            bcet, bcsc, mewl, mepd, merc, rery, mred, mres = cost_curves(
                data['BCET'], data['MCSC'], data['MEWDX'], data['MEWG'], data['MEWL'], data['MEPD'],
                data['MERC'], time_lag['MERC'], data['RERY'], data['MPTR'], data['MRED'], data['MRES'],
                titles['RTI'], titles['T2TI'], titles['ERTI'], year, dt, tech_to_resource
                )
