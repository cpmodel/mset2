...
def set_carbon_tax(data, c2ti, year):
    ...
    carbon_costs = (
                    ...
                    * data["REPPX"][:, :, 0]                              # Carbon price in USD / tCO2
                    * 1.3281 / ( data["PRSCX"][:, :, 0] * data["EX18"][:, :, 0] / (data["PRSC18"][:, :, 0]  * data["EXX"][:, :, 0]) )
                    / 1000                                                # Conversion from GWh to MWh
                    )
    ...
    if np.isnan(carbon_costs).any():
        ...
        print( ('Conversion factor:'
              f'{data["REX18"][33, 0, 0] / ( data["PRSCX"][:, :, 0] * data["EX18"][:, :, 0] / (data["PRSC18"][:, :, 0]  * data["EXX"][:, :, 0]) )}') )
...
def get_lcoe(data, titles):
    ...
    # Increase fuel prices with fuel price index
    fuel_cost = bcet[:, :, c2ti['5 Fuel ($/MWh)']] * data['FPIX'][:, :, 0]
    ft = get_cost_component(fuel_cost, 1, lt_mask)
    dft = get_cost_component(bcet[:, :, c2ti['6 std ($/MWh)']], 1, lt_mask)
    fft = get_cost_component(fuel_cost * data['MTFT'][:, :, 0], 1, lt_mask)
