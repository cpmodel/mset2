...
# Local library imports
from MINDSET_FTT_Power.SourceCode.support.divide import divide
...
def rldc(data, time_lag, data_dt, year, titles):
    ...
    elec_idx = titles['JTI'].index('8 Electricity')
    e_dem = data['MEWDX'][:, elec_idx, 0] /3.6*1000
    ...
    wind_idx = titles['T2TI'].index("Wind")
    solar_idx = titles['T2TI'].index("Solar")
    Sw = np.divide(data['MEWG'][:, wind_idx, 0],
                   e_dem[:],
                   where=~np.isclose(e_dem[:], 0.0))
    # Assume some very small generation if there is no wind at all
    Sw[Sw == 0] = 0.0000001
    Ss = np.divide(data['MEWG'][:, solar_idx, 0],
                   e_dem[:],
                   where=~np.isclose(e_dem[:], 0.0))
    # Assume some very small generation if there is no wind at all
    Ss[Ss == 0] = 0.0000001
    ...
    # Assume some very small generation if there is no wind at all
    vre_ggr_sol_with_min = vre_ggr_sol
    vre_ggr_sol_with_min[vre_ggr_sol_with_min == 0] = 0.000000002
    vre_ggr_sol = np.maximum((divide(data['MEWS'][:, solar_idx, 0] * data['MEWL'][:, solar_idx, 0],
                                 time_lag['MEWS'][:, solar_idx, 0] * time_lag['MEWL'][:, solar_idx, 0])
                                 -1.0)*data['MEWG'][:, solar_idx, 0],
                                 vre_ggr_sol_with_min)
    
    vre_gr_wind = 0.005 * np.ones(len(titles['RTI']))
    vre_gr_wind = np.maximum((divide(data['MEWS'][:, wind_idx, 0] * data['MEWL'][:, wind_idx, 0],
                                 time_lag['MEWS'][:, wind_idx, 0] * time_lag['MEWL'][:, wind_idx, 0])
                                 -1.0)*data['MEWS'][:, wind_idx, 0],
                                 0.005)
    
    vre_ggr_wind = 0.002 * data['MEWG'][:, wind_idx, 0]
    # Assume some very small generation if there is no wind at all
    vre_ggr_wind_with_min = vre_ggr_wind
    vre_ggr_wind_with_min[vre_ggr_wind_with_min == 0] = 0.000000002
    vre_ggr_wind = np.maximum((divide(data['MEWS'][:, wind_idx, 0] * data['MEWL'][:, wind_idx, 0],
                                 time_lag['MEWS'][:, wind_idx, 0] * time_lag['MEWL'][:, wind_idx, 0])
                                 -1.0)*data['MEWG'][:, wind_idx, 0],
                                 vre_ggr_wind_with_min)
    ...
    for r in range(len(titles['RTI'])):
        ...
        data['MCTG'][r, :, 0] = 0.0
        data['MCTG'][r, wind_idx, 0] = curt_w[r]
...
#            if abs(mlsc_split_solar) > 0.0 and abs(data['MLSC'][r,0,0] - mlsc_split_wind) > 0.0:
        ...
        if np.rint(data['MSAL'][r, 0, 0]) == 5:
            ...
            # Assign price values
            data['MSSP'][r, :,0] = 0.0
            data['MSSP'][r, wind_idx,0] = data['MSSR'][r,0,0] * SSw
            data['MSSP'][r, solar_idx,0] = data['MSSR'][r,0,0] * SSs
        # For option 4, all VRE pay the equal amount
        elif np.rint(data['MSAL'][r, 0, 0]) in [4]:
            data['MSSP'][r,:,0] = 0.0
            data['MSSP'][r,wind_idx,0] = data['MSSR'][r,0,0]
        ...
        if np.rint(data['MSAL'][r, 0, 0]) in [3, 4, 5]:
            ...
            vre_long = vre + data['MEWG'][r, solar_idx, 0]
            ...
            if np.rint(data['MSAL'][r, 0, 0]) in [3]:
                ...
                marg_cost_wind_ls = total_output_wind_l * data['MLCC'][r,0,0] / np.sum(data['MEWG'][r, :, 0] + vre_ggr_wind[r])  - data['MLSP'][r, wind_idx, 0]
            elif np.rint(data['MSAL'][r, 0, 0]) in [4]:
                marg_cost_sol_ls = total_output_solar_l * data['MLCC'][r,0,0] / (vre_long + vre_ggr_sol[r]) - data['MLSP'][r, solar_idx, 0]
                marg_cost_wind_ls = total_output_wind_l * data['MLCC'][r,0,0] / (vre_long + vre_ggr_wind[r]) - data['MLSP'][r, wind_idx, 0]
                ...
                marg_cost_wind_ls = total_output_wind_l * data['MLCC'][r,0,0] / (vre_long + vre_ggr_wind[r]) * LSw  - data['MLSP'][r, wind_idx, 0] 
            
            data['MLSM'][r, :, 0] = 0.0
            data['MLSM'][r, wind_idx, 0] = marg_cost_wind_ls
            ...
            if np.rint(data['MSAL'][r, 0, 0]) in [3]:
                ...
                marg_cost_wind_ss = output_wind * data['MSCC'][r,0,0] / np.sum(data['MEWG'][r, :, 0] + vre_ggr_wind[r])  - data['MSSP'][r, wind_idx, 0]
            elif np.rint(data['MSAL'][r, 0, 0]) in [4]:
                marg_cost_sol_ss = output_sol * data['MSCC'][r,0,0] / (vre + vre_ggr_sol[r]) - data['MSSP'][r, solar_idx, 0]
                marg_cost_wind_ss = output_wind * data['MSCC'][r,0,0] / (vre + vre_ggr_wind[r]) - data['MSSP'][r, wind_idx, 0]
            elif np.rint(data['MSAL'][r, 0, 0]) in [5]:
                marg_cost_sol_ss = output_sol * data['MSCC'][r,0,0] / (vre + vre_ggr_sol[r]) * SSs - data['MSSP'][r, solar_idx, 0] 
                marg_cost_wind_ss = output_wind * data['MSCC'][r,0,0] / (vre + vre_ggr_wind[r]) * SSw  - data['MSSP'][r, wind_idx, 0] 
                    
            data['MSSM'][r,:,0] = 0.0
            data['MSSM'][r,wind_idx,0] = marg_cost_wind_ss
