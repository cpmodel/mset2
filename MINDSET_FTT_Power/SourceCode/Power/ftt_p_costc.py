...
def marginal_function(MEPD, RERY, MPTR, BCSC, HistC, MRCL, MERC, MRED, MRES, dt):
    ...
    '''
    L = 990
    MRED = np.zeros([MEPD.shape[0], 14, 1])
    MRES = np.zeros([MEPD.shape[0], 14, 1])
    P = np.zeros([4])
    #dQdt = np.zeros([990])
    ...
    P[:4] = MRCL[1, :4, 0]     # All marginal costs of non-renewable resources are identical (global), we use Rest of Europe
    ...
    for j in range(4): # j is the non-renewable resource
        ...
        while abs((dFdt - demand_non_renewables[j]) / demand_non_renewables[j]) > 0.01  and count < 20:
            ...
            dFdt = np.sum(MPTR[:, j, 0].reshape((MEPD.shape[0], 1)) \
                         * BCSC[: , j, 4:] \
        ...
        BCSC[:, j, 4:] = BCSC[:, j, 4:] - (MPTR[:, j, 0].reshape((MEPD.shape[0], 1)) * BCSC[:, j, 4:] * (0.5 - 0.5 * np.tanh(1.25 * 2 * sig[j] * divide(HistC[j, :] - P[j], P[j])))) * dt
        RERY[:, j, 0] = np.sum((MPTR[:, j, 0].reshape((MEPD.shape[0], 1)) * BCSC[:, j, 4:] \
...
def cost_curves(BCET, BCSC, MEWD, MEWG, MEWL, MEPD, MERC, MRCL, RERY, MPTR, MRED, MRES, rti, t2ti, erti, year, dt, tech_to_resource):
    '''
    ...
    CSC_Q = np.zeros([BCSC.shape[0], 14, L])
    HistC = np.zeros([14, L])
    ...
    # Uranium
    MEPD[:, erti.index("1 Nuclear"), 0] = divide(MEWG[:, t2ti.index("Nuclear"), 0], BCET[:, 0, 13]) * 3.6 / 1000
    # Oil
    MEPD[:, erti.index("2 Oil"), 0] = MEWD[:, 2, 0] + MEWD[:, 3, 0] + MEWD[:, 4, 0]
    # Coal
    MEPD[:, erti.index("3 Coal"), 0] = MEWD[:, 0, 0] + MEWD[:, 1, 0]
    # Gas
    MEPD[:, erti.index("4 Gas"), 0] = MEWD[:, 6, 0]
    ...
    erti_bio_idx = erti.index("5 Biomass")
    bio_idx = t2ti.index("Bioenergy")
    bio_ccs_idx = t2ti.index("Bioenergy + CCS")
    MEPD[:, erti_bio_idx, 0] =  (divide(MEWG[:, bio_idx, 0],  BCET[:, bio_idx, 13])   \
                    + divide(MEWG[:, bio_ccs_idx, 0],  BCET[:, bio_ccs_idx, 13]))   \
                    * 3.6/1000 # +  MEWD[11, :]   +   MEWD[10, :]
    RERY[:, erti_bio_idx, 0] = MEPD[:, erti_bio_idx, 0]       # In PJ
    # Hydro
    MEPD[:, erti.index("9 Large Hydro"), 0] = MEWG[:, t2ti.index("Hydro"), 0] * 3.6/1000
    RERY[:, erti.index("9 Large Hydro"), 0] = MEPD[:, erti.index("9 Large Hydro"), 0] # in PJ
    # Wind
    MEPD[:, erti.index("10 Onshore"), 0] = MEWG[:, t2ti.index("Wind"), 0] * 3.6/1000
    RERY[:, erti.index("10 Onshore"), 0] = MEPD[:, erti.index("10 Onshore"), 0] # in PJ
    # Solar (PV + CSP)
    MEPD[:, erti.index("12 Solar PV"), 0] = (divide(MEWG[:, t2ti.index("Solar"), 0], BCET[:, t2ti.index("Solar"), 13])  +  divide(MEWG[:, t2ti.index("Solar"), 0], BCET[:, t2ti.index("Solar"), 13])) * 3.6/1000
    RERY[:, erti.index("12 Solar PV"), 0] = MEPD[:, erti.index("12 Solar PV"), 0] # in PJ 
    # Other renewable
    MEPD[:, erti.index("13 Geothermal"), 0] = MEWG[:, t2ti.index("Other Renewable"), 0] * 3.6/1000
    RERY[:, erti.index("13 Geothermal"), 0] = MEPD[:, erti.index("13 Geothermal"), 0] # in PJ
